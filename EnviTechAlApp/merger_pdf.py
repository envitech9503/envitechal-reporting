...
def get_report_mapping():
    ...
    return {
        ...
        'aa':  (AmbientAirForm,      ambientAirGeneratePDF1,       'Ambient Air'),
        'ww':  (WasteWaterSludge,    wasteWaterPdf1,               'Waste Water'),
        've':  (VehiculEmissionForm, vehicularEmissionReport1,     'Vehicular Emission'),
        'la':  (LuxAnalysisForm,     luxAnalysisReportPdf1,        'Lux Analysis'),
        'pp':  (PackingPolyBagForm,  packingPolyBagReport1,        'Packing Poly Bag'),
        'na':  (NoiseAnalysis,       noiseAnalysisReport1,         'Noise Analysis'),
        'mo':  (MachineOilForm,      machineOilReportPdf1,         'Machine Oil'),
        'ma':  (MicrobialAnalysis,   microbialAnalysisPdf1,        'Microbial Analysis'),
...
def get_pdf_bytes_for_report(lab_report_no, request):
    """Find report by lab_report_no (unique across all models) and return its PDF bytes."""
    mapping = get_report_mapping()

    for report_type, (model, generate_fn, report_name) in mapping.items():
        try:
            report = model.objects.get(lab_report_no=lab_report_no)
            print(f"✓ Found {lab_report_no} in {report_name}")
            report_id = report.id

            import inspect
            if 'return_bytes' in inspect.signature(generate_fn).parameters:
                pdf_bytes = generate_fn(request, report_id, return_bytes=True)
            else:
                response = generate_fn(request, report_id)
                if hasattr(response, 'content'):
                    pdf_bytes = BytesIO(response.content)
                else:
                    return False, "Function didn't return a valid response", report_name
            ...
            if pdf_bytes and isinstance(pdf_bytes, BytesIO):
                ...
                if pdf_bytes.read(4) == b'%PDF':
                    ...
                    return True, pdf_bytes, report_name
                else:
                    return False, "Generated content is not a valid PDF", report_name
            else:
                return False, f"PDF generation returned {type(pdf_bytes)}", report_name

        except model.DoesNotExist:
            continue
        except Exception as e:
            return False, f"Error in {report_name}: {str(e)}", report_name

    return False, f"Report {lab_report_no} not found in any model", None
...
def compress_pdf(pdf_bytes: BytesIO, target_mb: float = 5.0) -> BytesIO:
    ...
    if s3_mb > target_mb:
        print(
            ...
            f"       Tip: reduce the number of selected reports to stay under {target_mb} MB."
        )
...
def merge_pdfs(
    ...
    """
    Merge multiple lab reports (identified by lab_report_no) into a single
    password-protected, compressed PDF.
    """
    print(f"\n{'='*60}")
    print(f"  Merging {len(selected_reports)} report(s)")
    print(f"{'='*60}")

    merged_writer = PdfWriter()

    for idx, lab_report_no in enumerate(selected_reports, 1):
        if not lab_report_no:
            continue

        print(f"\n[{idx}] {lab_report_no}")
        success, result, report_name = get_pdf_bytes_for_report(lab_report_no, request)
        ...
        try:
            ...
            print(f"  ✓ Added {len(reader.pages)} page(s) — {report_name}")
